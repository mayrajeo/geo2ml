...
Docs: https://mayrajeo.github.io/geo2ml/data.coordinates.html.md"""
...
# %% ../../nbs/11_data.coordinates.ipynb #940cb185
import affine
...
# %% ../../nbs/11_data.coordinates.ipynb #da4b7119
def list_to_affine(xform_mat:list) -> affine.Affine:
    ...
    if len(xform_mat) > 6: xform_mat = xform_mat[:6]
    if rio.transform.tastes_like_gdal(xform_mat):
        return affine.Affine.from_gdal(*xform_mat)
    else:
        return affine.Affine(*xform_mat)

# %% ../../nbs/11_data.coordinates.ipynb #448bb233
def get_geo_transform(src) -> affine.Affine:
    "Extract geotransform for a raster image source"
    if isinstance(raster, str): return rio.open(raster_src).transform
    elif isinstance(raster_src, rio.DatasetReader): return raster_src.transform
    elif isinstance(raster_src, gdal.Dataset): return affine.Affine.from_gdal(*raster_src.GetGeoTransform())

# %% ../../nbs/11_data.coordinates.ipynb #8e5e4975
...
# %% ../../nbs/11_data.coordinates.ipynb #6dbd1004
...
# %% ../../nbs/11_data.coordinates.ipynb #9eefcb3a
def affine_transform_gdf(gdf:gpd.GeoDataFrame, 
    ...
    gdf["geometry"] = gdf["geometry"].map(lambda geom: convert_poly_coords(geom, affine_obj=affine_obj, inverse=inverse))
    if precision is not None:
        gdf['geometry'] = gdf['geometry'].map(lambda geom: _reduce_geom_precision(geom, precision=precision))
...
# %% ../../nbs/11_data.coordinates.ipynb #8f761040
...
# %% ../../nbs/11_data.coordinates.ipynb #38e8f366
